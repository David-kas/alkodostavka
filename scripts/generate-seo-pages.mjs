...
const WA_PREFILL = encodeURIComponent('Здравствуйте! Заказ АлкоДоставка 24, адрес: ');
const TITLE_TAIL = '| АлкоДоставка 24 · часто 20–40 мин · 18+';
const TITLE_TAIL_VOPROS = '— Москва · АлкоДоставка 24';
...
function raionIntro(name, slug) {
  ...
  const parts = [
    ...
    `Район ${n} входит в зону доставки АлкоДоставка 24 по Москве. Позвоните или напишите в мессенджер — подберём напитки под бюджет и подскажем, сколько ждать курьера с учётом пробок.`,
...
<header class="header site-header">
  ...
  <div class="container header-main-row">
    <a href="/" class="logo">АлкоДоставка 24</a>
...
function footerHtml() {
  return `<footer class="footer"><div class="container">
    <p>© 2026 АлкоДоставка 24. Доставка алкоголя в Москве. <strong>18+</strong></p>
...
function main() {
  ...
  for (const { name, slug } of raionEntries) {
    const p = `/raion/${slug}/`;
    const title = `Алкоголь с доставкой — ${name}, Москва ${TITLE_TAIL}`;
  ...
  for (const { name, slug, lineName } of metroEntries) {
    ...
    const title = `У метро «${name}», Москва — алкоголь на дом ${TITLE_TAIL}`;
  ...
  for (const k of KATEGORII) {
    ...
    const title = `${k.name} с доставкой по Москве ${TITLE_TAIL}`;
  ...
  for (const pv of POVODY) {
    ...
    const title = `Алкоголь на ${pv.name}, Москва ${TITLE_TAIL}`;
  ...
  for (const v of VOPROSY) {
    ...
    const title = `${v.q} ${TITLE_TAIL_VOPROS}`;
  ...
  writeFile(
    ...
    wrapPage({
      title: 'Алкоголь у метро Москвы ' + TITLE_TAIL,
  ...
  writeFile(
    ...
    wrapPage({
      title: 'Категории напитков с доставкой по Москве ' + TITLE_TAIL,
      description:
  ...
  writeFile(
    ...
    wrapPage({
      title: 'Алкоголь на праздник и событие, Москва ' + TITLE_TAIL,
      description:
  ...
  writeFile(
    ...
    wrapPage({
      title: 'Вопросы о доставке по Москве · АлкоДоставка 24',
  ...
  const rayonyPage = wrapPage({
    title: 'Алкоголь по районам Москвы ' + TITLE_TAIL,
