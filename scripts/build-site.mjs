...
function productCard(p) {
  ...
  return `<article class="card" data-brand="${brandOf(p.name)}" data-price="${p.price}">
    <a href="/product/${slug}.html" class="thumb">
      <picture><source srcset="${webp}" type="image/webp"><img loading="lazy" decoding="async" src="${p.image}" alt="${p.name}"></picture>
    </a>
...
const home = layout({
  ...
  canonicalPath: "/",
  content: `<section class="section"><h2>Преимущества сервиса</h2><ul><li>Оформление заказа без регистрации</li><li>Срочная доставка алкоголя и подтверждение оператором</li><li>Каталог на основе исходных данных: названия, категории, цены</li><li>Мобильный интерфейс и быстрые CTA-кнопки</li></ul></section>
<section class="section"><h2>Популярные товары</h2><div class="grid">${topProducts.map(productCard).join("")}</div></section>
